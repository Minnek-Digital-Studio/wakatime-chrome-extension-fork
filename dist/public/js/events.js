...
var WakaTimeCore = function () {
  ...
  _createClass(WakaTimeCore, [{
    ...
    value: function recordHeartbeat() {
      ...
      browser.storage.sync.get({
        ...
        if (items.loggingEnabled === true) {
          ...
          browser.idle.queryState(config.detectionIntervalInSeconds).then(function (newState) {
            if (newState === 'active') {
              // Get current tab URL.
              browser.tabs.query({ currentWindow: true, active: true }).then(function (tabs) {
                ...
                if (in_array(currentActiveTab.id, _this.tabsWithDevtoolsOpen)) {
                  debug = true;
                }
                ...
                if (items.loggingStyle == 'whitelist') {
                  var heartbeat = _this.getHeartbeat(currentActiveTab.url, items.whitelist);
                  console.log('active URL: ', currentActiveTab.url);
                  if (heartbeat.url) {
                    _this.sendHeartbeat(heartbeat, debug);
                  } else {
    ...
    key: 'getHeartbeat',
    value: function getHeartbeat(url, list) {
      var projectIndicatorCharacters = '@@';

      var lines = list.split('\n');
      for (var i = 0; i < lines.length; i++) {
        // strip (http:// or https://) and trailing (`/` or `@@`)
        var cleanLine = lines[i].trim().replace(/(\/|@@)$/, '').replace(/^(?:https?:\/\/)?/i, '');
    ...
    key: '_preparePayload',
    value: function _preparePayload(heartbeat, type) {
    ...
    key: 'sendHeartbeat',
    value: function sendHeartbeat(heartbeat, debug) {
      var _this2 = this;
      ...
      this._getLoggingType().done(function (loggingType) {
        ...
        if (loggingType == 'domain') {
          ...
          payload = _this2._preparePayload(heartbeat, 'domain', debug);
          console.log(payload);
          _this2.sendAjaxRequestToApi(payload);
        }
        // Send entity in heartbeat
        else if (loggingType == 'url') {
            payload = _this2._preparePayload(heartbeat, 'url', debug);
            console.log(payload);
            _this2.sendAjaxRequestToApi(payload);
          }
      });
